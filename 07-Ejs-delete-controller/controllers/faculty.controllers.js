import Faculty from "../models/faculty.model.js";

// home controller
export const home = (req, res) => {
  let studentName = "Ali";
  // I qyer my DB and received a array of friends
  const friends = [
    { name: "Ali", phone: 123 },
    { name: "Waqas", phone: 567 },
    { name: "Tayyab", phone: 789 },
  ];

  let number = 46;
  res.render("home", { studentName, friends, number });
};

// read all
export const readAllFaculty = async (req, res) => {
  try {
    // Submit fetch query to DB and get data in faculty array[]
    const faculty = await Faculty.find();
    const count = faculty.length;
    // return res.status(200).json({
    //   success: true,
    //   count,
    //   faculty,
    // });
    // console.log(faculty);

    return res.render("allFaculty.ejs", { faculty });
  } catch (error) {
    return res.status(500).json({
      success: faclse,
      msg: "Internal server error",
    });
  }
};

// read one info page
export const facultyInfoPage = async (req, res) => {
  res.render("details.ejs");
};

// read one
export const readOneFaculty = async (req, res) => {
  try {
    console.log(req.params);
    const { id } = req.params;

    const faculty = await Faculty.findById(id);

    if (!faculty) {
      return res.status(400).json({
        success: false,
        msg: `No Faculty exist with id=${id}`,
        // msg: "No Faculty exist with id= " + id,
      });
    }
    //  code to send a DB query to read a faculty data with id = 22920
    // return res.status(200).json({
    //   success: true,
    //   faculty,
    // });
    return res.render("details.ejs", { faculty });
  } catch (error) {
    return res.status(500).json({
      success: false,
      msg: "Internal server error",
    });
  }
};

// createNewFacultyForm
export const createNewFacultyForm = (req, res) => {
  res.render("createFaculty");
};

// create new facultry
export const createNewFaculty = async (req, res) => {
  try {
    // read data from Frontend and submit to DB
    console.log(req.body);

    const { name, employeeCode, salary, gender } = req.body;

    // new Faculty(req.body);
    const faculty = new Faculty({ name, employeeCode, salary, gender });

    // create query
    await faculty.save();

    // return res.status(201).json({
    //   success: true,
    //   faculty,
    // });

    return res.redirect("/faculty");
  } catch (error) {
    return res.status(500).json({
      success: false,
      msg: "Internal server error",
    });
  }
};

// update
export const updateFaculty = async (req, res) => {
  try {
    console.log(req.body);

    // read incoming data inside request and ask mongoDB to update specific faculty member
    const { id } = req.params;
    const faculty = await Faculty.findById(id);

    if (!faculty) {
      return res.status(400).json({
        success: false,
        msg: `No Faculty exist with id=${id}`,
        // msg: "No Faculty exist with id= " + id,
      });
    }

    const updatedFaculty = await Faculty.findByIdAndUpdate(
      id,
      { $set: req.body },
      { new: true },
    );

    // return res.status(200).json({
    //   success: true,
    //   updatedFaculty,
    // });

    return res.redirect("/faculty");
  } catch (error) {
    return res.status(500).json({
      success: faclse,
      msg: "Internal server error",
    });
  }
};

// update page render
export const updatePage = async (req, res) => {
  try {
    const { id } = req.params;
    const faculty = await Faculty.findById(id);

    if (!faculty) {
      return res.status(400).json({
        success: false,
        msg: `No Faculty exist with id=${id}`,
        // msg: "No Faculty exist with id= " + id,
      });
    }

    res.render("updateForm.ejs", { faculty });
  } catch (error) {
    return res.status(500).json({
      success: faclse,
      msg: "Internal server error",
    });
  }
};

// delete
export const deleteFaculty = async (req, res) => {
  try {
    // read incoming data inside request and ask mongoDB to update specific faculty member
    const { id } = req.params;

    const faculty = await Faculty.findById(id);

    if (!faculty) {
      return res.status(400).json({
        success: false,
        msg: `No Faculty exist with id=${id}`,
        // msg: "No Faculty exist with id= " + id,
      });
    }

    await Faculty.findByIdAndDelete(id);

    // return res.status(200).json({
    //   success: true,
    //   msg: "Faculty has been deleted",
    // });
    return res.redirect("/faculty");
  } catch (error) {
    return res.status(500).json({
      success: false,
      msg: "Internal server error",
    });
  }
};
